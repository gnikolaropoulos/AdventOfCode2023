...
class Mapping:
    ...
    def add_range(self, source, dest, length):
        ...
        if len(self._ranges) > end_insert_point:
            if self._ranges[end_insert_point][0] != source + length:
...
def solve_part_2(almanac):
    all_mapped = almanac.seed_ranges_to_location(almanac.seed_ranges)
    return min(all_mapped)[0]
